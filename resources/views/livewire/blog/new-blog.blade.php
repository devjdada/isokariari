...
<div>
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
        ...
        <x-form wire:submit="save">
            ...
            <x-select label="Category" wire:model='category' option-value="name" option-label="name" :options="$categories"
                single>
                <x-slot:append>
            ...
            <x-editor wire:model="content" :config="$config" label="Document" hint="The full product description" />
            <x-input label="Author" wire:model="author" />

            <x-slot:actions>
                <x-button label="Cancel" wire:click='reset' />
                <x-button label="Save Blog" class="btn-primary" type="submit" spinner="save" />
            </x-slot:actions>
        </x-form>
    </div>
